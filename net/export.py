...
def export_to_hpp(model_path="chess_value_net.pt", output_path="chess_net.hpp", scale=64):
    ...
    w1 = state_dict['hidden_layer.weight'].numpy()  # Shape: (16, 768)
    b1 = state_dict['hidden_layer.bias'].numpy()    # Shape: (16,)
    w2 = state_dict['output_layer.weight'].numpy()  # Shape: (1, 16)
...
// Hidden Layer Weights: Shape [16][768] (out_features x in_features)
const int16_t HIDDEN_WEIGHTS[16][768] = {{
{format_2d(w1_int)}
}};

// Hidden Layer Biases: Shape [16]
const int16_t HIDDEN_BIASES[16] = {{
    {format_1d(b1_int)}
}};

// Output Layer Weights: Shape [16] (Flattened out_features x in_features)
const int16_t OUTPUT_WEIGHTS[16] = {{
    {format_1d(w2_int[0])}
}};
...
if __name__ == "__main__":
    ...
    export_to_hpp(model_path="net7-4.pt", output_path="net7-4.hpp", scale=64)